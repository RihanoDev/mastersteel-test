...
func Authentication(c *fiber.Ctx) error {
	...
	err = db.QueryRow(context.TODO(), `SELECT id, email, password FROM service_users WHERE email = $1`, checkToken["email"]).Scan(&user.Id, &user.Email, &user.Password)
	if err != nil {
